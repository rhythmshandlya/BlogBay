...
const BlogPage = (props) => {
  const [numBlogs, numBlogsSetter] = useState(
  ...
  function Renderer(content) {
    ...
    if (isInCategory(content.category)) {
      return <EasyCard content={content} />;
    }
  }
  ...
  function CatBlogs() {
    return (
      <div
        className="sample-blogs"
        style={{
          gridTemplateColumns: 'repeat(' + (numBlogs - 1) + ',320px)',
          marginLeft: '20%'
        }}
      >
        {console.log(props.blogType[0])}
        {props.blogType[0] === 'Top Blog'
          ? props.blogs.map(TopRenderer)
          : props.blogs.map(Renderer)}
      </div>
    );
  }
  return (
